...
for scale = 1:numscales
    ...
    features = collect(conf, midres, conf.upsample_factor, conf.filters); %features = collect(conf, midres, conf.upsample_factor, conf.filters);
    clear midres
    ...
    hires = [hires collect(conf, patches, conf.upsample_factor, {})];  %hires = [hires collect(conf, patches, conf.scale, {})];
